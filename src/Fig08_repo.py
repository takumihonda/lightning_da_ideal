...
#    print( info_l )
    ...
    fn = '{0:}/data_info_l.npz'.format( data_path, )
    print( fn )
#    np.savez( fn, info_l=info_l, tvar_l=tvar_l )
    info_l = np.load( fn )['info_l']
    tvar_l = np.load( fn )['tvar_l']
...
#        r2d, z2d = np.meshgrid( r1d*0.001, z1d*0.001 )
        ...
        fn = '{0:}/data{1:0=2}.npz'.format( data_path, n )
        print( fn )
#        np.savez( fn, data=np.transpose(rz2d), x=r2d, y=z2d )
        data = np.load( fn )['data']
        x = np.load( fn )['x']
        y = np.load( fn )['y']

        #SHADE = ax.contourf( r2d, z2d, data, #np.transpose(rz2d),
        SHADE = ax.contourf( x, y, data,
                              cmap=cmap_rb, levels=levs,
                              extend='both' )

        #CONT = ax.contour( r2d, z2d, data, #np.transpose(rz2d),
        CONT = ax.contour( x, y, data,
