...
#                                  ew_m[zlev_tgt,:,:].shape )
    ...
    fn = '{0:}/data_max_loc_w.npz'.format( data_path, )
    #np.savez( fn, max_loc_w=max_loc_w )
    max_loc_w = np.load( fn )['max_loc_w']
...
#                 ax.clabel( CONT, inline=True, fontsize=6, fmt=r'%2.0f$^\circ$C',  )
       ...
       #          ax.set_xticks(np.arange(0,300,2), minor=False)
           ...
           fn = '{0:}/data_bar.npz'.format( data_path, )
           print( fn ) 
...
#           np.savez( fn, data=sumfp1d )
           data = np.load( fn )['data']
           #print(sumfp1d)

           #ax4.barh( INFO["Z"]*0.001, sumfp1d, color='k', height=0.4 )
           ax4.barh( INFO["Z"]*0.001, data, color='k', height=0.4 )
...
for tlev in range( tmin, tmax ):
    main( INFO, tlev=tlev, member=member, zlev_show=zlev_show, zlev_tgt=zlev_tgt, mem=mem, ftit=ftit )
